...
Laravel `Schema` [facade](/docs/laravel/facades) 对所有 Laravel 支持的数据库系统提供了创建和操作数据表的相应支持。


## 生成迁移

使用 [Artisan 命令](/docs/laravel/artisan)  `make:migration` 来创建迁移：
...
> {note} 如果你使用的是 [Homestead 虚拟机](/docs/laravel/homestead)，则应该在虚拟机中执行命令。
