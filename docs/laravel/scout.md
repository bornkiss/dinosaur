...
Laravel Scout 为 [Eloquent 模型](/docs/laravel/eloquent) 的全文搜索提供了一个简单的、基于驱动程序的解决方案。使用模型观察员，Scout 会自动同步你的搜索索引和 Eloquent 记录。
...
虽然不是严格要求使用 Scout，但在使用这个库之前，强烈建议你配置一个 [队列驱动程序](/docs/laravel/queues)。使用队列来对处理 Scout 将所有模型信息同步到搜索索引的操作，会极大的减少 Web 页面的响应时间。
...
如果你想通过 Eloquent 查询构造器将模型集合添加到搜索索引中，你也可以在 Eloquent 查询构造器上链式调用 `searchable` 方法。`searchable` 会把构造器的查询 [结果分块](/docs/laravel/eloquent#chunking-results) 并且将记录添加到你的搜索索引里。同样的，如果你已经配置 Scout 为使用队列，则所有的数据块将在后台由你的队列工作进程添加：
...
简单地使用 `delete` 从数据库中删除该模型就可以移除索引里的记录。这种删除形式甚至与 [软删除](/docs/laravel/eloquent#soft-deleting) 的模型兼容:
...
除了检索模型的集合，你也可以使用 `paginate` 方法对搜索结果进行分页。这个方法会返回一个就像 [传统的 Eloquent 查询分页](/docs/laravel/pagination) 一样的 `Paginator`  实例：
...
获取到检索结果后，就可以使用 [Blade](/docs/laravel/blade) 来渲染分页链接从而显示结果，就像传统的 Eloquent 查询分页一样：
