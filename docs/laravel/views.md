...
> {tip} 想寻找有关如何编写 Blade 模板的更多信息？查看完整的 [Blade 文档](/docs/laravel/blade)。
...
如你所见，`view` 函数中，传入的第一个参数对应着 `resources/views` 目录中视图文件的名称，第二个参数是可在视图文件中使用的数据数组。在示例中，我们传递 `name` 变量，该变量可以使用 [Blade 模板语言](/docs/laravel/blade) 在视图中显示。
...
在下面这个例子中，我们会在一个 [服务提供器](/docs/laravel/providers) 中注册视图合成器，使用 `View` Facade 来访问底层的 `Illuminate\Contracts\View\Factory` 契约实现。默认情况下，Laravel 没有存放视图合成器的目录，你需要根据喜好来重新建立目录，例如：`App\Http\ViewComposers`。
...
> {tip} 所有的视图合成器都会通过 [服务容器](/docs/laravel/container) 进行解析，所以你可以在视图合成器的构造函数中类型提示需要注入的依赖项。
